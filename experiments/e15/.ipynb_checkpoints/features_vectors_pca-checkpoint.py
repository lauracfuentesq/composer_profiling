...
def get_feature_vectors(dataloader, dataframe, set_type, feature_tensors):
    ...
    #Ensure model is in evaluation mode
        ...
        for i in tqdm(range(0,batch['input_ids'].shape[0],5), desc="computing feature tensors"):
            ...
            last_hidden_state_list.append(last_hidden_state.reshape(-1, 512))
    ...
    print(f'last hidden state list shape {last_hidden_state_list[0].shape}')

    ##======================== DO PCA =============================================##
    from sklearn.decomposition import PCA
    
    X=np.vstack(loader_tensor_list)
    ...
    full_array=np.zeros((dataframe.shape[0],1022,n_components))
    ...
    for element in range(0,full_array.shape[0]):
        
        flat_tensor=torch.flatten(torch.tensor(full_array[element]))
