...
def create_data_loader(dataframe):
  
  midi_paths = []
  labels=[]
  paths=dataframe['piano_scores_paths']
  ...
  for i,score in enumerate(paths):
      ...
      integer_label= 0 if dataframe['composer_gender'][i] == 'Male' else 1
  ...
  data_loader = DataLoader(dataset=dataset, collate_fn=collator, batch_size=len(dataframe))
